...
- **网络互联**：好友/同组成员自动建立 P2P（失败回退服务器中继）+ 虚拟网卡组成虚拟 LAN；用户资料里显示对接方式（P2P/中继）、虚拟 IP、收发字节、信号强度（按 RTT）。
- **网卡 TUN/TAP 可切换**（「网卡状态」里选）：
  - **TUN（L3，默认）**：按目的 IP 路由，无广播，最干净。
  - **TAP（L2）**：收发以太网帧，承载 **IPX 等非 IP 协议**，并支持**局域网广播发现**（老游戏找对局）。语义=本机广播 fanout 给所有对端一次、收到的广播只写本地网卡不再转发（学习式交换，无洪泛）。
  - 同一虚拟网内**所有人必须用相同模式**；TAP 需管理员运行，**Windows 还需装 tap-windows6（OpenVPN TAP）驱动**（TUN 用 Wintun 免驱动）。
